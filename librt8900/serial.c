...
void set_serial_attributes(int fd, bool flow_control)
{
        struct termios tty;
        ...
        if (flow_control) {
                tty.c_cflag |= CRTSCTS;
                log_msg(RT8900_DEBUG, "Flow control on\n");
        } else {
                tty.c_cflag &= ~CRTSCTS; /* no hardware flow control */
                log_msg(RT8900_DEBUG, "Flow control off\n");
        }
...
void open_serial(int *fd, char **serial_path, bool *dtr_pin_for_on)
{
        ...
        *fd = open(*serial_path, O_RDWR | O_NOCTTY );  //| O_NDELAY
        ...
        set_serial_attributes(*fd, *dtr_pin_for_on);
}